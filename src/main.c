...
static void day_update(struct tm *tick_time, TimeUnits units_changed) {
  update_date();
}
...
static void main_window_load(Window *window) {
  ...
  time_layer = text_layer_create(GRect(0, 20, 144, 60));
  ...
  date_layer = text_layer_create(GRect(0, 0, 72, 20));
  ...
  weather_layer = text_layer_create(GRect(0, 140, 144, 28));
  ...
  // Create city layer
  city_layer = text_layer_create(GRect(0,120, 144, 20));
...
static void init() {
  ...
  tick_timer_service_subscribe(DAY_UNIT, day_update);
